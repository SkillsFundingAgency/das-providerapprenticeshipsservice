...
namespace SFA.DAS.ProviderApprenticeshipsService.Web.Validation
{
    public class CsvRecordValidator : AbstractValidator<CsvRecord>
    {
        private static Func<string, IEnumerable<string>, bool> _inList = (v, l) 
            => string.IsNullOrWhiteSpace(v) || l.Contains(v);
        ...
        public CsvRecordValidator(BulkUploadApprenticeshipValidationText validationText)
        {
            ...
            RuleFor(r => r.FworkCode)
                .Cascade(CascadeMode.StopOnFirstFailure)
                .Must(m => m.TryParse() <= 999).When(m => !string.IsNullOrWhiteSpace(m.FworkCode))
                    .WithMessage(validationText.FworkCode01.Text).WithErrorCode(validationText.FworkCode01.ErrorCode)
                .Must(m => (m.TryParse() ?? 0) > 0).When(m => !string.IsNullOrWhiteSpace(m.ProgType) && _inList(m.ProgType, new[] { "2", "3", "20", "21", "22", "23" })).WithMessage(validationText.FworkCode02.Text).WithErrorCode(validationText.FworkCode02.ErrorCode);

            RuleFor(r => r.FworkCode)
                .Must(m => string.IsNullOrWhiteSpace(m) || m == "0").When(m => m.ProgType == "25").WithMessage(validationText.FworkCode03.Text).WithErrorCode(validationText.FworkCode03.ErrorCode);

            RuleFor(r => r.PwayCode)
                .Cascade(CascadeMode.StopOnFirstFailure)
                .Must(m => m.TryParse() <= 999)
                    .When(m => !string.IsNullOrWhiteSpace(m.PwayCode) && _inList(m.ProgType, new[]{ "2","3","20","21","22","23" }))
                    .WithMessage(validationText.PwayCode01.Text).WithErrorCode(validationText.PwayCode01.ErrorCode)
                .Must(m => (m.TryParse() ?? 0) > 0).When(m => !string.IsNullOrWhiteSpace(m.ProgType) && _inList(m.ProgType, new[] { "2", "3", "20", "21", "22", "23" })).WithMessage(validationText.PwayCode02.Text).WithErrorCode(validationText.PwayCode02.ErrorCode);

            RuleFor(r => r.PwayCode)
                .Must(m => string.IsNullOrWhiteSpace(m) || m == "0").When(m => m.ProgType == "25")
                    .WithMessage(validationText.PwayCode03.Text).WithErrorCode(validationText.PwayCode03.ErrorCode);

            RuleFor(r => r.StdCode)
                .Cascade(CascadeMode.StopOnFirstFailure)
                .Must(m => m.TryParse() <= 99999).When(m => !string.IsNullOrWhiteSpace(m.StdCode))
            ...
            RuleFor(r => r.StdCode)
                .Must(m => string.IsNullOrWhiteSpace(m) || m == "0").When(FrameworkProgTypeSelected)
