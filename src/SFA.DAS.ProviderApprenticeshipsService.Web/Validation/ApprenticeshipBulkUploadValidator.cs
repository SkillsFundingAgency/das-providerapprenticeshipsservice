    ...
    public sealed class ApprenticeshipBulkUploadValidator : ApprenticeshipBaseValidator
    {
        public ApprenticeshipBulkUploadValidator()
        {
            ...
            RuleFor(x => x.FirstName)
                ...
                .NotEmpty().Unless(m => m.FirstName == null).WithMessage(text.GivenNames02.Text).WithErrorCode(text.GivenNames02.ErrorCode)
                .NotNull().WithMessage(text.GivenNames01.Text).WithErrorCode(text.GivenNames01.ErrorCode);

            RuleFor(x => x.LastName)
                .Must(m => lengthLessThan(m, 100)).WithMessage(text.FamilyName02.Text).WithErrorCode(text.FamilyName02.ErrorCode)
                .NotEmpty().Unless(m => m.LastName == null).WithMessage(text.FamilyName02.Text).WithErrorCode(text.FamilyName02.ErrorCode)
                .NotNull().WithMessage(text.FamilyName01.Text).WithErrorCode(text.FamilyName01.ErrorCode);
            ...
            RuleFor(r => r.StartDate)
                .Must(base.ValidateDateWithoutDay).Unless(m => m.StartDate == null).WithMessage(text.LearnStartDate02.Text).WithErrorCode(text.LearnStartDate02.ErrorCode)
                .Must(m => CheckIfNotNull(m?.DateTime, m?.DateTime > now)).WithMessage("Learner start date must be in the future"); // ToDo: Delete?

            RuleFor(r => r.EndDate)
                .Must(ValidateDateWithoutDay).Unless(m => m.EndDate == null).WithMessage(text.LearnPlanEndDate02.Text).WithErrorCode(text.LearnPlanEndDate02.ErrorCode)
